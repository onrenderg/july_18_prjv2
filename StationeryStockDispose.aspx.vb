...
Public Class StationeryStockDispose
    Inherits SetCulture
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        lblTitle.Text = Resources.Resource.menuDisposeStationery
    ...
    Protected Sub save_button_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles save_button.Click
        ...
        For Each dr As GridViewRow In existing_grid.Rows
            ...
            Dim txtremarks As TextBox = DirectCast(dr.FindControl("txtremarks"), TextBox)
            ...
            If Val(qty.Text.ToString) > 0 Then
                ...
                drStationeryMapping("remarks") = txtremarks.Text
                tbl_StationeryMapping.Rows.Add(drStationeryMapping)
            End If
        Next
        ' Validation: Check for empty remarks when AllotedQuantity > 0
        For Each row As DataRow In tbl_StationeryMapping.Rows
            If row("AllotedQuantity") > 0 And String.IsNullOrEmpty(row("remarks").ToString()) Then

                'show a javascript alert.
                ClientScript.RegisterStartupScript(Me.GetType(), "alertscript", "alert('Remarks are required for entries with Quantity > 0.');", True)
        ...
        Try
            ...
            mytable = dbconnect.UpdateStationeryStock(tbl_StationeryMapping, LoginUser.DistrictCode, rbUnitType.SelectedValue, rbPriPost.SelectedValue, LoginUser.UserLevel, LoginUser.Code, LoginUser.Code, True, False)
            If mytable.Rows.Count > 0 Then
    ...
    Protected Sub cancel_button_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cancel_button.Click
        Response.Redirect("StationeryStockDispose.aspx")
    End Sub
    ...
    Protected Sub CustomValidator1_ServerValidate(ByVal source As Object, ByVal args As ServerValidateEventArgs)
        Dim row As GridViewRow = DirectCast(DirectCast(source, CustomValidator).NamingContainer, GridViewRow)
        Dim lblItemCounts As TextBox = DirectCast(row.FindControl("lblItemCounts"), TextBox)
        Dim txtremarks As TextBox = DirectCast(row.FindControl("txtremarks"), TextBox)

        If Not String.IsNullOrEmpty(lblItemCounts.Text) And String.IsNullOrEmpty(txtremarks.Text) Then
            args.IsValid = False
        Else
            args.IsValid = True
        End If
